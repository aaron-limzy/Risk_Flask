...
$sql = "SELECT `SERVER`,TICKET,LOGIN,TYPE,SYMBOL, VOLUME,OPEN_TIME,ROUND(OPEN_PRICE,5) AS OPEN_PRICE,CLOSE_TIME,ROUND(CLOSE_PRICE,5) AS CLOSE_PRICE,REASON,ROUND(SWAPS,2) AS SWAPS,ROUND(PROFIT,2) AS PROFIT,`GROUP` FROM
(
...
FROM live1.mt4_trades WHERE LEFT(SYMBOL,6) = '".$Symbol."' AND CLOSE_TIME > DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d 23:00:00'),INTERVAL 1 DAY) AND CMD < 2 
AND (mt4_trades.`GROUP` IN (SELECT `GROUP` FROM live5.group_table WHERE BOOK = 'A'))
UNION
...
CASE WHEN mt4_trades.REASON = 0 THEN 'Client' WHEN mt4_trades.REASON = 1 THEN 'Expert' WHEN mt4_trades.REASON = 2 THEN 'Dealer' WHEN mt4_trades.REASON = 3 THEN 'Signal' WHEN mt4_trades.REASON = 4 THEN '4' WHEN mt4_trades.REASON = 5 THEN 'Mobile' ELSE 00 END AS REASON,SWAPS AS SWAPS,PROFIT AS PROFIT
FROM live2.mt4_trades WHERE LEFT(SYMBOL,6) = '".$Symbol."' AND CLOSE_TIME > DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d 23:00:00'),INTERVAL 1 DAY) AND CMD < 2 
AND (mt4_trades.`GROUP` IN (SELECT `GROUP` FROM live5.group_table WHERE BOOK = 'A'))
UNION
...
CASE WHEN mt4_trades.REASON = 0 THEN 'Client' WHEN mt4_trades.REASON = 1 THEN 'Expert' WHEN mt4_trades.REASON = 2 THEN 'Dealer' WHEN mt4_trades.REASON = 3 THEN 'Signal' WHEN mt4_trades.REASON = 4 THEN '4' WHEN mt4_trades.REASON = 5 THEN 'Mobile' ELSE 00 END AS REASON,SWAPS AS SWAPS,PROFIT AS PROFIT
FROM live3.mt4_trades WHERE LEFT(SYMBOL,6) = '".$Symbol."' AND CLOSE_TIME > DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d 23:00:00'),INTERVAL 1 DAY) AND CMD < 2 
AND (mt4_trades.`GROUP` IN (SELECT `GROUP` FROM live5.group_table WHERE BOOK = 'A'))
UNION
...
CASE WHEN mt4_trades.REASON = 0 THEN 'Client' WHEN mt4_trades.REASON = 1 THEN 'Expert' WHEN mt4_trades.REASON = 2 THEN 'Dealer' WHEN mt4_trades.REASON = 3 THEN 'Signal' WHEN mt4_trades.REASON = 4 THEN '4' WHEN mt4_trades.REASON = 5 THEN 'Mobile' ELSE 00 END AS REASON,SWAPS AS SWAPS,PROFIT AS PROFIT
FROM live5.mt4_trades WHERE LEFT(SYMBOL,6) = '".$Symbol."' AND CLOSE_TIME > DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00') AND CMD < 2 
AND (mt4_trades.`GROUP` IN (SELECT `GROUP` FROM live5.group_table WHERE BOOK = 'A'))
...
<script type="text/javascript">
 window.onload=function(){
               var table = document.getElementById("fx");//获取第一个表格  
